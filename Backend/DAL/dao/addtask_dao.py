...
class EmployeeTasksDAO:
    ...
    @staticmethod
    async def create_employee_task(db: AsyncSession, request):
    ...
    async def update_employee_task(db, task_uuid, request):
        # Find task
        result = await db.execute(
            select(EmployeeTasks).where(EmployeeTasks.task_uuid == task_uuid)
        )
    ...
    async def get_all_employee_tasks(db):
        result = await db.execute(select(EmployeeTasks))
    ...
    async def get_tasks_by_user(db, user_uuid: str):
        result = await db.execute(
            select(EmployeeTasks).where(EmployeeTasks.user_uuid == user_uuid)
        )
