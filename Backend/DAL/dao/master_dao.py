...
class CountryDAO:
    ...
    async def create_country(self, uuid: str, calling_code: str, country_name: str):
        new_country = Countries(
            country_uuid=uuid, country_name=country_name, calling_code=calling_code
        )
...
class EducationDAO:
    ...
    async def get_education_level_by_eduname(self, education_name):
        result = await self.db.execute(
            select(EducationLevel).where(
                EducationLevel.education_name == education_name
            )
        )
    ...
    async def create_education_level(
        self, request_data: CreateEducLevelRequest, uuid: str
    ):
    ...
    async def get_education_level_by_eduname_and_uuid(
        self, education_name: str, education_uuid: str
    ):
        result = await self.db.execute(
            select(EducationLevel)
            .where(EducationLevel.education_name == education_name)
    ...
    async def create_education_country_mapping(
        self, educ_level_uuid, educ_doc_uuid, country_uuid, uuid
    ):
    ...
    async def check_education_country_mapping(
        self, educ_level_uuid, educ_doc_uuid, country_uuid
    ):
        result = await self.db.execute(
            select(CountryEducationDocumentMapping)
            .where(
                CountryEducationDocumentMapping.education_document_uuid == educ_doc_uuid
            )
            .where(CountryEducationDocumentMapping.country_uuid == country_uuid)
            .where(CountryEducationDocumentMapping.education_uuid == educ_level_uuid)
        )
    ...
    async def get_education_country_mapping_by_uuid(self, mappng_uuid):
        result = await self.db.execute(
            select(CountryEducationDocumentMapping).where(
                CountryEducationDocumentMapping.mapping_uuid == mappng_uuid
            )
        )
        return result.scalar_one_or_none()
...
class ContactDAO:
    ...
    async def get_contact_by_uuid(self, uuid):
        result = await self.db.execute(
            select(Contacts).where(Contacts.contact_uuid == uuid)
        )
        return result.scalar_one_or_none()
