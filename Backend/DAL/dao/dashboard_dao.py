...
class DashboardDAO:
    ...
    async def get_dashboard_summary(self, start_date=None, end_date=None):
        ...
        candidates_ever_accepted = (
            accepted + submitted + verified
            + joining_pending + joining + rescheduled + completed_offers
        )
        # Candidates who were actually sent an offer (exclude "Created" — offer not sent yet)
        candidates_ever_offered = total - created

        acceptance_rate = (candidates_ever_accepted / candidates_ever_offered * 100) if candidates_ever_offered else 0
        ...
        doc_configs = [
            ...
            ("education",  EmployeeEducationDocument.user_uuid, EmployeeEducationDocument.status),
            ("identity",   EmployeeIdentityDocument.user_uuid,  EmployeeIdentityDocument.status),
        ...
        overall_doc_pct = round(
            (total_distinct_verified / (total * len(doc_configs))) * 100, 2
        ) if total else 0
        ...
        doc_summary_pct   = round(
            (agg_verified_rows / doc_summary_total) * 100, 2
        ) if doc_summary_total else 0
        ...
        now = datetime.utcnow()
        _active_statuses = (
            "Created", "Offered", "Accepted", "Submitted",
            "Verified", "Joining Pending", "Joining", "Rescheduled",
        )
        ...
        recent_result = await self.db.execute(
            ...
            .outerjoin(Departments, Departments.department_uuid == EmployeeDetails.department_uuid)
            .outerjoin(ManagerEmployee, ManagerEmployee.employee_id == OfferLetterDetails.reporting_manager)
        ...
        active_candidates    = created + offered + accepted + submitted + verified + joining_pending + joining + rescheduled
        completed_candidates = completed_offers
        pending_candidates   = rejected

        # Priority classification of each action item
        def _priority(count: int, high_threshold: int, medium_threshold: int = 1) -> str:
        ...
        return {
            "birthdays": [
                self._format_celebration_item(
                    employee,
                    self._get_celebration_date(employee.date_of_birth, start_date, end_date)
                )
    ...
    def _format_celebration_item(self, employee, event_date):
        return {
            "name": " ".join(
                part for part in [employee.first_name, employee.middle_name, employee.last_name]
                if part
            ),
    ...
    def _format_work_anniversary_item(self, employee, start_date: date, end_date: date):
        anniversary_date = self._get_celebration_date(employee.joining_date, start_date, end_date)
        return {
