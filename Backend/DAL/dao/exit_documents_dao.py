...
class ExitDocumentsDAO:
    async def get_exit(self, db, exit_uuid):
        ...
        query = select(EmployeeExit).where(EmployeeExit.exit_uuid == exit_uuid)
        result = await db.execute(query)

        return result.scalars().first()

    async def create_document(
        self, db, exit_uuid, employee_uuid, document_type, file_name, file_path, user_id
    ):
    ...
    async def get_documents(self, db, exit_uuid):
        ...
        query = select(ExitDocuments).where(ExitDocuments.exit_uuid == exit_uuid)
