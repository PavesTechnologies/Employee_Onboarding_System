...
class AuditMiddleware(BaseHTTPMiddleware):
    def __init__(self, app):
        ...
        self.open_endpoints = [
            "/docs",
            "/openapi.json",
            "/ems/openapi.json" "/redoc",
            "/login",
            "/health",
            "/ems/masters/country",
            "/ems/employee-details",
            "/ems/employee-upload",
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if method in ["GET", "OPTIONS"] or any(
            path.startswith(p) for p in self.open_endpoints
        ):
        ...
        entity_name, entity_id = AuditUtils().extract_entity_info(
            path, req_body, method
        )
        ...
        old_data = None
        print(
            f"Entity Name: {entity_name}, Entity_ID: {entity_id}, Operation: {operation}"
        )
        ...
        if 200 <= response.status_code < 300:
            ...
            if new_data and operation == "CREATE":
                ...
                if new_entity_id:
                    async for db in get_db():
                        new_entity_data = await AuditUtils().get_data(
                            db, entity_name, new_entity_id
                        )
                        break
