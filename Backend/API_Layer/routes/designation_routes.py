...
router = APIRouter(prefix="/masters/designations", tags=["Designations"])
...
async def create_designation(
    data: DesignationCreate, db: AsyncSession = Depends(get_db)
):
...
async def get_designations_by_department(
    ...
    return await DesignationsService.get_designations_by_department(db, department_uuid)
...
async def get_designation_name(
    designation_uuid: str, db: AsyncSession = Depends(get_db)
):
...
async def update_designation(
    designation_uuid: str, data: DesignationUpdate, db: AsyncSession = Depends(get_db)
):
    designation = await DesignationsService.update_designation(
        db, designation_uuid, data
    )
    if not designation:
...
async def delete_designation(designation_uuid: str, db: AsyncSession = Depends(get_db)):
    designation = await DesignationsService.delete_designation(db, designation_uuid)
    if not designation:
        raise HTTPException(status_code=404, detail="Designation not found")
