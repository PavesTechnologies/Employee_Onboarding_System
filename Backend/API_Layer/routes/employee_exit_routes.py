...
from Backend.API_Layer.interfaces.employee_exit_interface import (
    EmployeeExitCreate,
    EmployeeExitResponse,
)

router = APIRouter(prefix="/employee-exit", tags=["Employee Exit"])


# create a employee exit
@router.post("/create", response_model=EmployeeExitResponse)
async def create_employee_exit(
    data: EmployeeExitCreate, db: AsyncSession = Depends(get_db)
):
    try:
...
async def get_all_employee_exits(db: AsyncSession = Depends(get_db)):
    try:
        service = EmployeeExitService()
...
async def update_employee_exit(
    employee_uuid: str, data: EmployeeExitCreate, db: AsyncSession = Depends(get_db)
):
    try:
        service = EmployeeExitService()
        result = await service.update_employee_exit_by_employee_uuid(
            db, employee_uuid, data
        )
        return result
    except HTTPException as he:
        raise he
...
async def delete_employee_exit(exit_uuid: str, db: AsyncSession = Depends(get_db)):
    try:
        service = EmployeeExitService()
