...
from Backend.API_Layer.interfaces.departments_interface import (
    DepartmentCreate,
    DepartmentResponse,
    DepartmentUpdate,
)
...
async def update_department(
    department_uuid: str, data: DepartmentUpdate, db: AsyncSession = Depends(get_db)
):

    department = await DepartmentsService.update_department(db, department_uuid, data)
...
async def get_department_by_uuid(
    department_uuid: str, db: AsyncSession = Depends(get_db)
):

    department = await DepartmentsService.get_department_by_uuid(db, department_uuid)
...
async def delete_department(department_uuid: str, db: AsyncSession = Depends(get_db)):

    department = await DepartmentsService.delete_department(db, department_uuid)
