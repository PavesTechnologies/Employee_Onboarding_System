...
from Backend.API_Layer.interfaces.exit_approval_interface import (
    ExitApprovalCreate,
    ManagerApprovalRequest,
    ExitApprovalResponse,
    HRApprovalRequest,
)
from fastapi import Request
from Backend.API_Layer.utils.role_based import require_roles

router = APIRouter(prefix="/exit-approvals", tags=["Exit Approvals"])


@router.post("/create")
async def create_exit_approval(
    data: ExitApprovalCreate, db: AsyncSession = Depends(get_db)
):
    try:
...
async def manager_approve(
    ...
    try:
        ...
        result = await service.manager_approve(
            db, payload.approval_uuid, user_id, payload.status, payload.remarks
        )
...
async def hr_approve(
    payload: HRApprovalRequest, request: Request, db: AsyncSession = Depends(get_db)
):
    try:
        user = request.state.user
...
async def get_all_approvals(db: AsyncSession = Depends(get_db)):
    try:
        service = ExitApprovalService()
...
async def get_my_pending_approvals(
    ...
    try:
        ...
        result = await service.get_my_pending_approvals(db, role)
...
async def get_approvals_by_exit_uuid(
    exit_uuid: str, db: AsyncSession = Depends(get_db)
):
    try:
        service = ExitApprovalService()
...
async def get_approvals_history(exit_uuid: str, db: AsyncSession = Depends(get_db)):
    try:
        service = ExitApprovalService()
