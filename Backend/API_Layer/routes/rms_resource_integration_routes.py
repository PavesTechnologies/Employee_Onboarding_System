...
async def get_rms_resources(db: AsyncSession = Depends(get_db)):
    ...
    stmt = (
        ...
        .outerjoin(
            EmployeeExit, EmployeeDetails.employee_uuid == EmployeeExit.employee_uuid
        )
    )
        ...
        response.append(
            {
                ...
                "experience": (
                    float(resource.experience) if resource.experience else None
                ),
                ...
                "annual_ctc": (
                    float(resource.annual_ctc) if resource.annual_ctc else None
                ),
