...
from ..interfaces.master_interfaces import (
    ...
    CountryDetails,
    CountryAllDetails,
    CreateEducLevelResponse,
    CreateEducLevelRequest,
    EducLevelDetails,
    AllEducLevelDetails,
    CountryEductionMapping,
    CountryEducationMappingDetails,
    CreateContactResponse,
    CreateContactRequest,
    ContactDetails,
    UpdateContactRequest,
)
from ...Business_Layer.services.master_services import (
    CountryService,
    EducationService,
    ContactService,
)
...
async def create_country(calling_code: str, db: AsyncSession = Depends(get_db)):
    try:
        ...
        return CreateCountryResponse(
            message="Created Country Successfully", country_uuid=country_id
        )
...
async def get_country_uuid(country_uuid: str, db: AsyncSession = Depends(get_db)):
    try:
        country_service = CountryService(db)
...
async def create_education_level(
    request_data: CreateEducLevelRequest, db: AsyncSession = Depends(get_db)
):
    try:
...
async def get_all_education_levels(db: AsyncSession = Depends(get_db)):
    try:
        education_service = EducationService(db)
...
async def update_education_level(
    ...
    try:
        education_service = EducationService(db)
        result = await education_service.update_education_level(
            request_data, education_uuid
        )
        return CreateEducLevelResponse(
            education_uuid=result.education_uuid,
...
@router.delete(
    "/education-level/{education_uuid}", response_model=CreateCountryResponse
)
...
@router.post(
    ...
    response_model=CountryEductionMapping,
)
async def create_education_country_mapping(
    ...
    try:
        education_service = EducationService(db)
        result = await education_service.create_education_country_mapping(
            educ_level_uuid, educ_doc_uuid, country_uuid
        )
...
async def delete_education_country_mapping(
    mapping_uuid: str, db: AsyncSession = Depends(get_db)
):
    try:
        education_service = EducationService(db)
...
async def get_all_education_country_mapping(db: AsyncSession = Depends(get_db)):
    try:
        education_service = EducationService(db)
...
async def create_contact(
    request_data: CreateContactRequest, db: AsyncSession = Depends(get_db)
):
    try:
        contact_service = ContactService(db)
        result = await contact_service.create_contact(request_data)
        return CreateContactResponse(
            contact_uuid=result.contact_uuid, message="Contact Created Successfully"
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def delete_contact(contact_uuid: str, db: AsyncSession = Depends(get_db)):
    try:
        ...
        return CreateContactResponse(
            contact_uuid=result.contact_uuid, message="Contact Deleted Successfully"
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def update_contact(
    ...
    try:
        ...
        result = await contact_service.update_contact(contact_uuid, request_data)

        return CreateContactResponse(
            contact_uuid=result.contact_uuid, message="Contact Updated Successfully"
        )
