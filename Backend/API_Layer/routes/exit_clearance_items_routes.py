...
from Backend.Business_Layer.services.exit_clearance_items_service import (
    ExitClearanceItemsService,
)
...
router = APIRouter(prefix="/exit-clearance-items", tags=["Exit Clearance Items"])


@router.post("/create")
async def create_items(
    data: ExitClearanceItemCreate, db: AsyncSession = Depends(get_db)
):
    try:
...
async def get_items(clearance_uuid: str, db: AsyncSession = Depends(get_db)):
...
async def update_item(
    data: ExitClearanceItemUpdate, request: Request, db: AsyncSession = Depends(get_db)
):
...
@router.get("/all", response_model=list[ExitClearanceItemResponse])
