...
from ...API_Layer.interfaces.education_interfaces import (
    CountryEducationMappingResponse,
    CreateEducDocRequest,
    EducDocResponse,
    DeleteEmpEducResponse,
    EducDocDetails,
    UploadFileResponse,
    EmployeEduDocDetails,
    DegreeMasterResponse,
    DegreeMasterRequest,
)
...
@router.post(
    ...
    response_model=EducDocResponse,
    dependencies=[Depends(require_roles("HR", "ADMIN"))],
)
async def create_education_document(
    request_data: CreateEducDocRequest, db: AsyncSession = Depends(get_db)
):
    try:
...
async def update_education_document(
    uuid: str, request_data: CreateEducDocRequest, db: AsyncSession = Depends(get_db)
):
    try:
        education_service = EducationDocService(db)
...
async def delete_education_document_by_uuid(
...
async def create_employee_education_document(
    ...
    try:
        ...
        result, uuid = await education_service.create_employee_education_document(
            request_data, file
        )
...
@router.put(
    "/employee-education-document/{document_uuid}", response_model=UploadFileResponse
)
async def update_employee_education_document(
    ...
    try:
        ...
        print(
            f"Time taken to update employee education document: {end_time - start_time}"
        )
        return UploadFileResponse(
...
@router.get(
    "/employee-education-document/{document_uuid}",
    response_model=EmployeEduDocDetails,
    dependencies=[Depends(require_roles("HR", "ADMIN", "GENERAL"))],
)
async def get_employee_education_document_by_uuid(
    uuid: str, db: AsyncSession = Depends(get_db)
):
    try:
        education_service = EducationDocService(db)
...
@router.delete(
    "/employee-education-document/{uuid}",
    response_model=DeleteEmpEducResponse,
    dependencies=[Depends(require_roles("HR", "ADMIN", "GENERAL"))],
)
async def delete_employee_education_document_by_uuid(
    uuid: str, db: AsyncSession = Depends(get_db)
):
    try:
        education_service = EducationDocService(db)
        result = await education_service.delete_employee_education_document_by_uuid(
            uuid
        )
...
@router.get(
    "/country-mapping/{country_uuid}",
    response_model=list[CountryEducationMappingResponse],
)
async def get_education_identity_mappings_by_country_uuid(
    country_uuid: str, db: AsyncSession = Depends(get_db)
):
    try:
        print("In route")
        education_service = EducationDocService(db)
        result = (
            await education_service.get_education_identity_mappings_by_country_uuid(
                country_uuid
            )
        )
...
async def create_degree_master(
    request_data: DegreeMasterRequest, db: AsyncSession = Depends(get_db)
):
    try:
        education_service = EducationDocService(db)
...
@router.get(
    "/degree-master/{education_uuid}", response_model=list[DegreeMasterResponse]
)
async def get_degree_master_by_education_uuid(
    education_uuid: str, db: AsyncSession = Depends(get_db)
):
    try:
        education_service = EducationDocService(db)
        result = await education_service.get_degree_master_by_education_uuid(
            education_uuid
        )
        return result
    except HTTPException as he:
