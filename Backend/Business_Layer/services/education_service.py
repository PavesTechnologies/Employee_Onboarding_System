...
class EducationDocService:
    ...
    async def update_education_document(self, uuid, request_data):
        try:
            ...
            if request_data.document_name:
                ...
                existing = await self.dao.get_document_by_name_and_uuid(
                    document_name, uuid
                )
                if existing:
                    raise HTTPException(
    ...
    async def create_employee_education_document(self, request_data, file):
        ...
        try:
            ...
            if not await self.dao.education_mapping_exists(
                request_data["mapping_uuid"]
            ):
    ...
    async def update_employee_education_document(
        self, document_uuid, request_data, file
    ):
    ...
    async def get_employee_education_document_by_uuid(self, uuid):
        try:
            ...
            if not result:
                raise HTTPException(
                    status_code=200,
                    detail="No Employee Education Document Found for this Employee",
                )
            return result
        except HTTPException as he:
            raise he
    ...
    async def get_education_identity_mappings_by_country_uuid(self, country_uuid):
        ...
        t2 = time.perf_counter()
        result = await self.dao.get_education_identity_mappings_by_country_uuid(
            country_uuid
        )
