...
async def get_dashboard_data(db, start_date: date, end_date: date):
    ...
    # ================= DB UPDATE LOGIC =================
    # 🔴 joining_date < today AND status != Completed → Joining Pending
    result = await db.execute(
        update(OfferLetterDetails)
        .where(
            OfferLetterDetails.joining_date < today,
            OfferLetterDetails.status == "Joining"   # only update active joining
        )
...
# # 🔴 UPDATE DB: Joining → JOINING_PENDING
#     result = await db.execute(
#         update(OfferLetterDetails)
#         .where(
#             OfferLetterDetails.joining_date < today,
