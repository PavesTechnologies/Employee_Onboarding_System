...
class EmployeeExitService:
    ...
    async def create_employee_exit(self, db, data):
        try:
            existing = await self.get_employee_exit_by_employee_uuid(
                db, data.employee_uuid
            )

            if existing:
                raise HTTPException(
                    status_code=400, detail="Employee exit already exists"
                )
    ...
    async def get_employee_exit_by_employee_uuid(self, db, employee_uuid):
        try:
            return await self.dao.get_employee_exit_by_employee_uuid(db, employee_uuid)
    ...
    async def get_employee_exit_by_exit_uuid(self, db, exit_uuid):
        try:
            result = await self.dao.get_employee_exit_by_exit_uuid(db, exit_uuid)
    ...
    async def update_employee_exit_by_employee_uuid(self, db, employee_uuid, data):
        try:
            existing = await self.get_employee_exit_by_employee_uuid(db, employee_uuid)

            if not existing:
                raise HTTPException(status_code=404, detail="Employee exit not found")

            return await self.dao.update_employee_exit_by_employee_uuid(
    ...
    async def update_employee_exit_by_exit_uuid(self, db, exit_uuid, data):
        try:
            existing = await self.get_employee_exit_by_exit_uuid(db, exit_uuid)
