...
class IdentityService:
    ...
    async def get_identity_type_by_uuid(self, uuid):
        try:
            ...
            if not result:
                raise HTTPException(
                    status_code=200, detail="No Identity Type Found for this employee"
                )
            return result
        except HTTPException as he:
            raise he
    ...
    async def create_identity_type(self, request_data):
        try:
            existing = await self.dao.get_identity_type_by_name(
                request_data.identity_type_name
            )
            if existing:
                raise HTTPException(
                    status_code=422,
                    detail="Identity Type with this name already exists",
                )
            uuid = generate_uuid7()
    ...
    async def create_country_identity_mapping(self, request_data):
        try:
            ...
            existing = await self.dao.get_identity_type_by_uuid(
                request_data.identity_type_uuid
            )
            if not existing:
                raise HTTPException(status_code=404, detail="Identity Type Not Found")

            mapping_existing = await self.dao.get_country_identity_mapping(
                request_data.country_uuid, request_data.identity_type_uuid
            )
            if mapping_existing:
                raise HTTPException(
                    status_code=422,
                    detail="Mapping already exists for this country and identity type",
                )
            uuid = generate_uuid7()
    ...
    async def get_country_identity_mapping_by_uuid(self, uuid):
        try:
            ...
            if not result:
                raise HTTPException(
                    status_code=200, detail="No Country Identity Mapping Found"
                )
            return result
        except HTTPException as he:
            raise he
    ...
    async def delete_country_identity_mapping(self, uuid):
        ...
        employee_existing = (
            await self.dao.get_employee_identity_documents_by_mapping_uuid(uuid)
        )
    ...
    async def get_identities_by_country(self, country_uuid: str):
        ...
        if not rows:
            raise HTTPException(
                status_code=404, detail="Country not found or no identities configured"
            )
