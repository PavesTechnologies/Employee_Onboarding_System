...
class HrOnboardingService:
    ...
    async def final_submit_onboarding(self, user_uuid):
        try:
            ...
            # Candidate email
            send_candidate_onboarding_submitted_email(
                to_email=offer["mail"], candidate_name=candidate_name
            )
    ...
    async def view_onboarding_documents(self, file_path: str):
        try:
            ...
            if len(names) == 2:
                ...
                table_name = names[0]
                existing = await self.dao.identity_and_education_document_exists(
                ...
                table_name = names[0]
                col_name = names[1]
                existing = await self.dao.experience_document_exists(
                    table_name, col_name, file_path
                )
                if not existing:
                    raise HTTPException(status_code=404, detail="Document Not Found")
            else:
    ...
    async def update_verification_status(
        self, user_uuid: str, status: str, current_user_id: int
    ):
        """
        ...
        if not updated:
            raise HTTPException(
                status_code=404, detail="Offer letter not found for the given user UUID"
            )
