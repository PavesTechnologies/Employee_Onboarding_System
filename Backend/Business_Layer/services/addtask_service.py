...
class EmployeeTasksService:
    ...
    async def create_employee_task(db, request):
        ...
        task = await EmployeeTasksDAO.create_employee_task(db, request)

        if not task:
            raise HTTPException(
                status_code=404, detail="User UUID not found in offer letter records"
            )
    ...
    async def update_employee_task(db, task_uuid, request):
        task = await EmployeeTasksDAO.update_employee_task(db, task_uuid, request)
    ...
    async def get_all_employee_tasks(db):
        ...
        return EmployeeTaskListResponse(
            ...
            tasks=[EmployeeTaskItemResponse.model_validate(task) for task in tasks],
        )
