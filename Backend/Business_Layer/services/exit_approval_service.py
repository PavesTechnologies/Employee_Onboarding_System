...
class ExitApprovalService:
    ...
    async def manager_approve(self, db, approval_uuid, user_id, status, remarks):
        try:
            ...
            approval = await self.dao.manager_approve(
                db, approval_uuid, user_id, status, remarks
            )

            if not approval:
                raise HTTPException(
                    status_code=404, detail="Manager approval not found"
                )
    ...
    async def hr_approve(self, db, approval_uuid, user_id, status, remarks):
        try:
            ...
            if not approval:
                raise HTTPException(status_code=404, detail="HR approval not found")
